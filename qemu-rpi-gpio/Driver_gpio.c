...
int init_GPIO_module( void )
{
	...
	printk(KERN_INFO "Driver_GPIO: Inicio Sensor %s seleccionado por defecto", shared_file);
        ...
        if (proc_entry == NULL) {
            ...
            printk(KERN_INFO "GPIO_module: Driver Cargado\n");
        }
    }
...
void exit_GPIO_module( void )
{
    ...
    vfree(shared_file);
    printk(KERN_INFO "Driver_GPIO: Descarga finalizada\n");
}
